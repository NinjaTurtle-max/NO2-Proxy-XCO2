"""
=============================================================
 ERA5 기상 재분석 다운로드 → 즉시 슬라이싱 → CSV 저장 → 원본 삭제
 - stream=oper, type=an, levtype=sfc
 - 변수: u10, v10, blh
 - 대상 연도: 2024 (Phase 1)
   → 2020/2022/2023은 L2 추가 다운 후 Phase 2에서 처리
 - 대상 지역: 동아시아 (20–50°N, 100–150°E)
=============================================================
 ★ 수정 이력 (v1 → v2)
   [BUG-1] time_key 탐색에 "valid_time" 추가
           ERA5 new CDS API는 time 대신 valid_time 사용
   [BUG-2] YEARS = ["2024"] 로 고정
           tropomi_dict에 없는 연도는 전부 SKIP되던 문제 해결
           (2020/2022/2023은 Phase 2에서 별도 처리)
   [BUG-3] slice_nc_to_csv 내부 격자 3중 for loop → numpy 벡터화
           121×201 격자 × 시간 × 일 → Python 루프 제거
           meshgrid 기반 배열 연산으로 대체 (속도 수십~수백 배 향상)
=============================================================
 사전 준비:
   pip install cdsapi netCDF4 pandas numpy

   .cdsapirc 파일 생성 후 아래 내용 추가
...
CSV_DIR = config.ERA5_CSV_DIR
TROPOMI_CSV_PATH = config.S5P_DIR / "time_check" / "tropomi_time_check.csv"

NC_DIR.mkdir(parents=True, exist_ok=True)
CSV_DIR.mkdir(parents=True, exist_ok=True)

# [BUG-2 수정] Phase 1: 2024년만 처리
YEARS  = ["2024"]
...
def get_tropomi_dict(csv_path: Path) -> dict:
    """
    파일명에서 날짜(YYYYMMDD)와 hour_mean 파싱
    반환: { year: { month: { day: set([hours]) } } }
    """
    print("📋 TROPOMI CSV 시간 정보 로딩 중...")
    if not csv_path.exists():
        print(f"❌ TROPOMI CSV 없음: {csv_path}")
        return {}

    df = pd.read_csv(csv_path)
    df['date'] = df['file'].str.extract(r'____(\d{8})T')
    df = df.dropna(subset=['date', 'hour_mean'])

    tropomi_dict = {}
    for _, row in df.iterrows():
        d_str = row['date']
        y, m, d = d_str[0:4], d_str[4:6], d_str[6:8]
        hr = int(round(row['hour_mean']))

        tropomi_dict.setdefault(y, {}).setdefault(m, {}).setdefault(d, set()).add(hr)

    # 연도별 일자 수 출력
    for yr, yd in sorted(tropomi_dict.items()):
        n_days = sum(len(dd) for dd in yd.values())
        print(f"  {yr}년: {n_days}일치 관측 데이터")
...
def verify_netcdf(fpath: Path) -> tuple[bool, str]:
    ...
    try:
        with nc.Dataset(fpath, "r") as ds:
            ...
            nan_ratio = float(np.sum(np.isnan(u10.filled(np.nan))) / u10.size)
            if nan_ratio > 0.99:
...
# ─────────────────────────────────────────────
# NC → CSV 슬라이싱 (numpy 벡터화, valid_time 대응)
# ─────────────────────────────────────────────
def slice_nc_to_csv(
    nc_path: Path, csv_path: Path,
    month_dict: dict, year: str, month: str
) -> tuple[bool, str, int]:
    """
    [BUG-1] valid_time / time 양쪽 탐색
    [BUG-3] 격자 루프 제거 → numpy meshgrid + 배열 슬라이싱
    """
    try:
        with nc.Dataset(nc_path, "r") as ds:
            ...
            if not all([lat_key, lon_key, time_key]):
                return False, f"좌표 변수 탐색 실패 (lat={lat_key}, lon={lon_key}, time={time_key})", 0
            ...
            def extract_sub(var_key):
                ...
                # [BUG-3] 도메인 슬라이싱을 여기서 수행 → 이후 연산 크기 대폭 축소
                if raw.ndim == 3:
                    return raw[:, lat_idx[0]:lat_idx[-1]+1, lon_idx[0]:lon_idx[-1]+1]
                elif raw.ndim == 2:
                    return raw[lat_idx[0]:lat_idx[-1]+1, lon_idx[0]:lon_idx[-1]+1]
                return raw

            u10_sub = extract_sub("u10")   # (T, lat, lon)
            ...
            # [BUG-3] meshgrid로 (lat, lon) 좌표 배열 미리 생성
            lon_grid, lat_grid = np.meshgrid(lon_sub, lat_sub)  # (n_lat, n_lon)
            ...
            for d_str, hours_set in month_dict.items():
                ...
                # ── 일평균 (벡터화) ──
                u10_daily_2d = np.nanmean(u10_sub[day_indices], axis=0)  # (lat, lon)
                v10_daily_2d = np.nanmean(v10_sub[day_indices], axis=0)
                blh_daily_2d = np.nanmean(blh_sub[day_indices], axis=0) if blh_sub is not None else None

                u10_d_flat = u10_daily_2d.ravel()
                v10_d_flat = v10_daily_2d.ravel()
                blh_d_flat = blh_daily_2d.ravel() if blh_daily_2d is not None else np.full(len(u10_d_flat), np.nan)
                ...
                for obs_h in sorted(hours_set):
                    ...
                    blh_o_flat = blh_sub[ti].ravel() if blh_sub is not None else np.full(len(u10_o_flat), np.nan)
                    ...
                    wd_obs   = (np.degrees(np.arctan2(u10_o_flat, v10_o_flat)) + 360) % 360
                    ws_daily = np.sqrt(u10_d_flat**2 + v10_d_flat**2)
                    wd_daily = (np.degrees(np.arctan2(u10_d_flat, v10_d_flat)) + 360) % 360
...
# 단일 월 처리
# ─────────────────────────────────────────────
def process_month(client, year: str, month: str, month_dict: dict) -> dict:
    ...
    target_days = sorted(list(month_dict.keys()))

    for attempt in range(1, MAX_RETRY + 1):
        result["attempts"] = attempt
        print(f"\n  [{attempt}/{MAX_RETRY}] {year}-{month} ({len(target_days)}일치)")
        ...
        ok, msg, n_rows = slice_nc_to_csv(nc_path, csv_path, month_dict, year, month)
        if not ok:
            print(f"❌ {msg}")
        ...
        result.update({
            "status": "OK", "csv_rows": n_rows,
            "csv_kb": round(csv_kb, 1), "error": "",
        })
        return result
...
def main():
    ...
    results = []
    print(f"\n▶ Phase 1: ERA5 다운로드 ({', '.join(YEARS)}년)")
    ...
    if failed_n > 0:
        fail_df = df[df["status"] == "FAILED"][["year","month","attempts","nc_mb","error"]]
...
def retry_from_csv(fail_csv_path: str):
    ...
    for _, row in df.iterrows():
        y, m = str(row["year"]), str(row["month"]).zfill(2)
...
if __name__ == "__main__":
    ...
    # 실패 재시도 시 아래 주석 해제
    # retry_from_csv(r"E:\dataset\ERA5\csv\era5_failed_list.csv")